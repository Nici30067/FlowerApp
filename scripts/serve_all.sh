...
(TRAVEL_PORT=8002 TRAVEL_EXECUTION_BACKEND=local TRAVEL_AGENT_MODE=rules TRAVEL_DATA_MODE=fixture \
  TRAVEL_DB=runtime/offline.sqlite3 nohup ./scripts/serve_supergrid.sh > "$LOG_DIR/server-8002-offline.log" 2>&1 &)
...
print('backend:', c['execution_backend'], '| agent mode:', c['agent_mode'], '| data:', c['data_mode'],
      '| model:', c['model'], '| federation:', (c.get('flower') or {}).get('federation'))
"
done
