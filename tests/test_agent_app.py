...
from travel_agent.schemas import Proposal, TripSnapshot
...
def test_scenario_plan_then_rain_commits_between_steps(runtime):
    ...
    assert types.count("travel.agent.completed") >= 9  # five specialists, then a discovery re-run after the rain request
...
def test_job_mode_returns_proposal_as_run_event(runtime, baseline):
    ...
    assert proposal.proposed.revision == snapshot.revision + 1 and result["metrics"]["model_calls"] >= 5
...
def test_flower_bundle_configuration():
    ...
    assert {"input", "model", "max-tool-turns", "model-timeout-s", "wall-time-s", "public-web"} <= set(agent)
