...
from travel_agent.planning.engine import points_for, rainy, schedule_order, search, validate
...
class Coordinator:
    ...
    def plan(self, original: TripSnapshot, event: TripEvent | None = None) -> Proposal:
        ...
        def run(role, matrix=None):
            return record(self.runner.run(role, work, ToolDispatcher(work, matrix, self.connectors)))

        req = work.request
        now = max(req.start, work.progress.now or req.start)
        # One protocol for every runner. When rain is already in the forecast, Conditions speaks first so that
        # Discovery consumes its request in a single pass. Otherwise Discovery and Conditions read the same
        # evidence and neither depends on the other's first report, so they run concurrently on a frozen copy;
        # their reports are appended in a fixed order for deterministic state.
        if rainy(work.weather, now, req.end, req.rain_threshold_pct):
            ...
            if any(m.recipient == "discovery" for m in conditions.requests):
                discovery = run("discovery")
        self.budget.consume("provider")
        points = points_for(work)
        matrix = self.provider.matrix(points, req.transport_mode)
        mobility = run("mobility", matrix)

        # The specialists' ranking is load-bearing: the planner searches what they ranked, minus what they said to
        # avoid. Required, reserved and completed places are never avoided. The catalog is a disclosed fallback.
        protected = set(req.required_place_ids) | {r.place_id for r in req.reservations}
        protected |= set(work.progress.completed_place_ids)
        avoided = {pid for report in work.reports for pid in report.avoid_ids} - protected
        catalog_order = [p.id for p in work.places]
        ranking = [pid for pid in dict.fromkeys(mobility.candidate_ids + discovery.candidate_ids) if pid not in avoided]
        source = "specialists"
        if not ranking:
            ranked = bool(mobility.candidate_ids or discovery.candidate_ids)
            reason = "no unavoided specialist candidates" if ranked else "no specialist candidates"
            ranking = [pid for pid in catalog_order if pid not in avoided]
            source = "catalog"
            self.emit("planner.fallback", {"reason": reason, "candidate_count": len(ranking)})
        self.emit("planner.started", {"algorithm": "bounded_beam_search", "beam_width": 64,
                                      "candidate_count": len(ranking), "avoided_count": len(avoided),
                                      "ranking_source": source})

        def short(plan):
            return not plan.validation.valid or len(plan.stops) < req.min_stops

        previous = work.itinerary  # the plan every search in this step is scored against
        itinerary = search(work, matrix, ranking)
        remaining = [pid for pid in catalog_order if pid not in ranking and pid not in avoided]
        if short(itinerary) and remaining:
            ranking = ranking + remaining
            self.emit("planner.fallback", {"reason": "specialist ranking infeasible", "candidate_count": len(ranking)})
            itinerary = search(work, matrix, ranking)
        if short(itinerary) and avoided:
            ranking = ranking + [pid for pid in catalog_order if pid in avoided]
            self.emit("planner.fallback", {"reason": "avoided places reconsidered", "candidate_count": len(ranking)})
            itinerary = search(work, matrix, ranking)
        first_score = itinerary.score  # computed while work.itinerary was still `previous`
        work.itinerary = itinerary
        budget_review = run("budget_pace", matrix)
        # A reviewer can request one extra candidate-order repair. It cannot relax user constraints, and it is
        # a fair comparison: the alternative is scored against the same previous plan the first search saw.
        addressed = any(m.recipient in ("discovery", "coordinator") for m in budget_review.requests)
        if addressed and budget_review.candidate_ids:
            # The reviewer's own avoid list is honoured too (it reported after `avoided` was computed).
            late = set(budget_review.avoid_ids) - protected
            preferred = [pid for pid in dict.fromkeys(budget_review.candidate_ids + ranking)
                         if (pid in ranking or pid not in avoided) and pid not in late]
            work.itinerary = previous
            try:
                alternative = search(work, matrix, preferred)
            finally:
                work.itinerary = itinerary
            identical = [s.place_id for s in alternative.stops] == [s.place_id for s in itinerary.stops]
            if not alternative.validation.valid:
                verdict = "alternative infeasible"
            elif identical:
                verdict = "alternative identical to the proposal"
            elif itinerary.validation.valid and alternative.score < first_score:
                verdict = "alternative scored lower"
            else:
                verdict = "alternative adopted"
            self.emit("planner.repair", {"adopted": verdict == "alternative adopted", "reason": verdict,
                                         "proposal_score": round(first_score, 3),
                                         "alternative_score": round(alternative.score, 3)})
            if verdict == "alternative adopted":
